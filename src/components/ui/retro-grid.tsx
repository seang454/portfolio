...
const DEFAULT_ANIMATION_DURATION_SECONDS = 15
...
void main() {
  ...
  float gridScrollSpeed = (gridHeight * gridTravelRatio) / max(u_animation_duration, 0.001);
...
interface RetroGridProps extends HTMLAttributes<HTMLDivElement> {
  ...
  /**
   * Grid scroll animation duration in seconds
   * @default 15
   */
...
interface ProgramInfo {
  ...
  uniforms: {
    ...
    animationDuration: WebGLUniformLocation
...
function getProgramInfo(
  ...
  const animationDuration = gl.getUniformLocation(
    program,
...
function createFallbackGridStyle(
  ...
  return {
    animation: `${FALLBACK_ANIMATION_NAME} ${animationDuration}s linear infinite`,
...
export function RetroGrid({
  ...
  }, [angle, animationDuration, cellSize, darkLineColor, lightLineColor])
  ...
  useEffect(() => {
    ...
    const draw = (timestamp: number) => {
      ...
      gl.uniform1f(
        programInfo.uniforms.animationDuration,
        Math.max(animationDurationRef.current, 0.001)
      )
      gl.uniform1f(
